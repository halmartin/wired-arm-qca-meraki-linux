...
static char *meraki_get_model(void)
{
	...
	if (strstr(model, "MR46E")) {
		return "mr46e/";
	}
	if (strstr(model, "MR46")) {
		return "mr46/";
	}
	if (strstr(model, "MR56")) {
		return "mr56/";
	}
	if (strstr(model, "MR36")) {
		return "mr36/";
	}
	if (strstr(model, "MR76")) {
		return "mr76/";
	}
	if (strstr(model, "MR86")) {
		return "mr86/";
	}
	if (strstr(model, "MR44")) {
		return "mr44/";
	}
		...
		uint8_t is_end, uint8_t bdf_type)
{
	...
	case BDF_TYPE_GOLDEN:
		snprintf(filename, sizeof(filename),
				"IPQ8074/%s" DEFAULT_BDF_FILE_NAME, meraki_get_model());
		break;
