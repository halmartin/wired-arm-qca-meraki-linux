...
bool edma_stp_rstp;
bool edma_add_ath_hdr_rx;   // Meraki: add Atheros header to received frames
	...
	do {
		while (sw_next_to_clean != hw_next_to_clean) {
			...
			if ((unlikely(!port_id)) || (unlikely(port_id > EDMA_MAX_PORTID_SUPPORTED))) {
				dev_err(&pdev->dev, "Invalid RRD source port bit set %d", port_id);
			...
			/*
			 * Meraki: Following the edma_stp_rstp example below, we
			 * optionally add an Atheros header to all received frames.
			 */
			if (edma_add_ath_hdr_rx) {
				u16 port_type = (rd->rrd1 >> EDMA_RRD_PORT_TYPE_SHIFT) & EDMA_RRD_PORT_TYPE_MASK;
				u16 ath_hdr = (EDMA_RX_ATH_HDR_VERSION << EDMA_RX_ATH_HDR_VERSION_SHIFT) |
					    (priority << EDMA_RX_ATH_HDR_PRIORITY_SHIFT) |
					    (port_type << EDMA_RX_ATH_PORT_TYPE_SHIFT) | port_id;

				skb_put(skb, 4);
				memmove(&skb->data[EDMA_ETH_HDR_LEN + 4],
					&skb->data[EDMA_ETH_HDR_LEN],
					length - EDMA_ETH_HDR_LEN);

				*(uint16_t *)&skb->data[12] = htons(edma_ath_eth_type);
				*(uint16_t *)&skb->data[14] = htons(ath_hdr);
			} else if (unlikely(edma_stp_rstp)) {
	...
	bool packet_is_rstp, int nr_frags)
{
	...
	if (unlikely(packet_is_rstp)) {
		...
		word3 |= 1 << EDMA_TPD_FROM_CPU_SHIFT;
	}
...
/*
 * edma_set_add_ath_hdr_rx()
 *	set add_ath_hdr_rx
 */
void edma_set_add_ath_hdr_rx(bool add_ath_hdr_rx)
{
	edma_add_ath_hdr_rx = add_ath_hdr_rx;
}
