...
#define	DUMP_COMPLETE_TIMEOUT	20000

static struct timer_list dump_timeout;
static struct completion dump_complete;
static struct timer_list dump_complete_timeout;
...
volatile int q6_ubi_done;
struct kobject *q6_kobj;

static ssize_t q6_ubi_done_store(struct kobject *kobj,
                                 struct kobj_attribute *attr,
                                 const char *buf,
                                 size_t count)
{
	if (!timer_pending(&dump_complete_timeout)) {
		pr_warn("We're not saving q6mem right now!\n");
		return count;
	}

	pr_warn("Dump complete. Notifying driver...\n");
	del_timer_sync(&dump_complete_timeout);

	if (!completion_done(&dump_complete))
		complete(&dump_complete);

	return count;
}

struct kobj_attribute q6_ubi_done_attr = __ATTR_WO(q6_ubi_done);
...
static void dump_complete_timeout_func(unsigned long data)
{
	dump_complete_timeout.data = -ETIMEDOUT;
	complete(&dump_complete);
}
...
static int q6_dump_release(struct inode *inode, struct file *file)
{
	...
	/* User-space released q6mem dev node but we still have to copy it
	 * out to UBI. User-space will notify us when it's done but we want
	 * an upper bound. Start a timer so we don't stall indefinitely.
	 */
	setup_timer(&dump_complete_timeout, dump_complete_timeout_func, 0);
	mod_timer(&dump_complete_timeout, jiffies + msecs_to_jiffies(DUMP_COMPLETE_TIMEOUT));
...
static int q6_rproc_probe(struct platform_device *pdev)
{
	...
	q6_ubi_done = 0;
	q6_kobj = kobject_create_and_add("q6v5-wcss", kernel_kobj);
	if (sysfs_create_file(q6_kobj, &q6_ubi_done_attr.attr)) {
		pr_err("Could not create /sys/kernel/q6v5-wcss/q6_ubi_done !!\n");
		goto free_rproc;
	}
