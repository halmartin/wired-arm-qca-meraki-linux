...
static ssize_t
qfprom_show_serial_num(struct device *dev,
		       struct device_attribute *attr,
		       char *buf)
{
	ssize_t ret;
	phys_addr_t qfprom_pa;
	unsigned long qfprom_sz;
	void __iomem *qfprom_ioaddr;
	ssize_t qfprom_offset;

	if (of_machine_is_compatible("qcom,ipq4019")) {
		qfprom_pa = 0x00058000;
		qfprom_sz = 0xD0;
		qfprom_offset = 0x08;
	} else if (of_machine_is_compatible("qcom,ipq807x")) {
		qfprom_pa = 0x000A0000;
		qfprom_sz = 0x200;
		qfprom_offset = 0x128;
	} else {
		// should never happen
	...
	qfprom_ioaddr = ioremap_nocache(qfprom_pa, qfprom_sz);
	if (!qfprom_ioaddr)
		return -ENOMEM;
	ret = snprintf(buf, 16, "0x%08X\n", readl(qfprom_ioaddr + qfprom_offset));
	iounmap(qfprom_ioaddr);

	return ret;
}

static struct device_attribute serial_attr =
	__ATTR(serial_num, 0444, qfprom_show_serial_num, NULL);

static int qfprom_create_serial_attr(struct device *dev)
{
	int err = 0;

	if (of_machine_is_compatible("qcom,ipq4019") || of_machine_is_compatible("qcom,ipq807x")) {
		err = device_create_file(dev, &serial_attr);
		if (err)
			pr_err("%s: %d\n", __func__, err);
	}
...
static int __init qfprom_create_files(int size, int16_t sw_bitmap)
{
	...
	err = qfprom_create_serial_attr(&device_qfprom);
	if (err)
		return err;
