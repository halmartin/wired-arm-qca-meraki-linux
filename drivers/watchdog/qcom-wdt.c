...
// bit 0 RST_STS 0x1: wdt (the last system reset was due to WDT0)
#define QCOM_WDT_RST_STS (1 << 0)
...
static int qcom_wdt_probe(struct platform_device *pdev)
{
	...
	/*
	 * Check if hw watchdog bite triggered
	 * (cf drivers/soc/qcom/scm_restart_reason.c:)
	 */
	reason = qcom_get_restart_reason();
	...
	if (reason > 0)
		wdt->wdd.bootstatus = reason;
	else
		wdt->wdd.bootstatus = (readl(wdt_addr(wdt, WDT_STS)) & QCOM_WDT_RST_STS);
	dev_info(&pdev->dev, "boot reason:%d\n", wdt->wdd.bootstatus);
...
#ifndef CONFIG_ARM64
	/*
	 * Do not use DT machine restart since we have registered a restart notifier
	 */
	arm_pm_restart = NULL;
#endif
