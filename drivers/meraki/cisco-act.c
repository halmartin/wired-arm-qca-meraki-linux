...
static void act_hardware_reset(struct act_data *act)
{
	if (act == NULL || act->dev == NULL) {
		dev_err(act->dev, "No act driver data\n");
		return;
	}

	if (act->chip_family != TAM_FAMILY_ACT2) {
		dev_err(act->dev, "Hard reset not available on this device\n");
		return;
	}

	if (act->reset_gpio == NULL) {
		dev_err(act->dev, "Reset GPIO uninitialized\n");
		return;
	}

	gpiod_set_value_cansleep(act->reset_gpio, 1);
	udelay(1000);
	gpiod_set_value_cansleep(act->reset_gpio, 0);
}

static ssize_t cisco_act_store_hardware_reset(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t count)
{
	struct act_data *act = dev_get_drvdata(dev);
	...
	if (act == NULL) {
		dev_err(dev, "No act driver data\n");
	} else if (act->chip_family != TAM_FAMILY_ACT2) {
		dev_err(dev, "Hard reset not available on this device\n");
	} else {
		act_hardware_reset(act);
	}

	return count;
}
static DEVICE_ATTR(hardware_reset, 0200, NULL, cisco_act_store_hardware_reset);
					...
					const struct cisco_act_bus_ops* bus_ops)
{
	...
	if (act->chip_family == TAM_FAMILY_ACT2) {
		...
		act->reset_gpio = reset_gpio;

		if (reset_gpio == NULL) {
			dev_err(dev, "Could not initialize ACT2 reset GPIO\n");
		}
	} else {
		act->reset_gpio = NULL;
	}
	...
	if (act->chip_family == TAM_FAMILY_ACT2) {
		// Attempt to reset chip to get to known state after kernel init
		act_hardware_reset(act);
	}
