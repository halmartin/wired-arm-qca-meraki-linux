...
int __qcom_scm_dload_read(struct device *dev, u32 svc_id, u32 cmd_id, void *resp_buf)
{
	struct qcom_scm_desc desc = {0};
	struct arm_smccc_res res;
	int ret;

#define TCSR_DLOAD_MAGIC_REG	0x193D100ull

	desc.args[0] = TCSR_DLOAD_MAGIC_REG;
	ret = qcom_scm_call(dev, ARM_SMCCC_OWNER_SIP, SCM_SVC_IO_ACCESS, SCM_IO_READ, &desc, &res);
	*(uint32_t *)resp_buf = res.a1;
